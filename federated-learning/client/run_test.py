...
def main():
    ...
    model = load_bert_tiny_model(config['model_path'], device)
    print('-------- Model loaded --------')
    ...
    test_loader = create_test_dataloader(config['dataset'], tokenizer, config['test_batch_size'], config['max_length'], config['seed'])
    print(f'Test Loader: {len(test_loader.dataset)} total sentences. {len(test_loader)} batches of size {config["test_batch_size"]}.')
