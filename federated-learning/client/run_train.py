...
def main():
    ...
    train_loader, eval_loader = create_dataloaders(config['dataset'], tokenizer, config['train_size'], config['eval_size'], config['train_batch_size'], config['eval_batch_size'], config['max_length'], config['seed'])
    ...
    print('\n-------- Creating Optimizer --------')
    optimizer = create_optimizer(config['optimizer'], model, config['learning_rate'])
    print('-------- Optimizer created --------')
    
    print('\n-------- Creating Scheduler --------')
    num_training_steps = config['num_epochs'] * len(train_loader)
    scheduler = create_scheduler(config['scheduler'], optimizer, num_training_steps, config['scheduler_warmup_steps'])
    print('-------- Scheduler created --------')
    
    print('\n-------- Training --------')
    train_text_class(model, config['models_path'], config['model_name'], train_loader, eval_loader, optimizer, config['learning_rate'], scheduler, config['num_epochs'], device, config['eval_flag'], config['progress_bar_flag'])
