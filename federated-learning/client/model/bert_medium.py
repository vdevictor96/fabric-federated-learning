...
def get_bert_medium_model(device='cuda'):
    model = AutoModelForSequenceClassification.from_pretrained(
    "prajjwal1/bert-medium")  # v1 and v2
    model.to(device)
    return model

def load_bert_medium_model(model_path, device='cuda'):
    model = AutoModelForSequenceClassification.from_pretrained(
    "prajjwal1/bert-medium")  # v1 and v2
